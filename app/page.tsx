    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-blue-50 text-slate-900">
        ...
        <div className="mx-auto flex max-w-7xl flex-col px-6 pb-20 pt-8 sm:px-8 lg:px-12">
          <header className="mb-16 flex items-center justify-between rounded-full border border-white/60 bg-white/70 px-5 py-3 shadow-sm backdrop-blur">
            ...
            <nav className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
              <a href="#idea" className="transition hover:text-slate-900">About</a>
              ...
              <a href="#future" className="transition hover:text-slate-900">Future</a>
            </nav>
          </header>
      ...
      <section id="idea" className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-12">
        <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <InfoCard icon={Brain} title="Traditional LMS" text="Mainly focuses on course delivery, announcements, assignment submission, and grade tracking." />
            <InfoCard icon={Sparkles} title="AI-driven LMS" text="Adds prediction, personalization, learning recommendations, and continuous improvement based on student data." />
            <InfoCard icon={GraduationCap} title="Personalization" text="Supports different needs, abilities, learning speeds, and academic goals instead of treating students as one group." />
            <InfoCard icon={Building2} title="Institutional value" text="Helps schools and universities make better interventions, manage outcomes, and allocate support more effectively." />
          </div>
        </div>
      </section>
      ...
      <section id="impact" className="mx-auto max-w-7xl px-6 py-24 sm:px-8 lg:px-12">
        <div className="grid gap-12 lg:grid-cols-[1fr_1fr]">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <InfoCard icon={LineChart} title="Benefits" text="Better engagement, stronger support, earlier intervention, and more efficient use of institutional resources." />
            <InfoCard icon={ShieldAlert} title="Risks" text="Privacy issues, algorithmic bias, dependence on data quality, and the possibility of over-relying on automation." />
            <InfoCard icon={Building2} title="Costs" text="Development, system integration, staff training, data protection compliance, and long-term maintenance." />
            <InfoCard icon={Sparkles} title="Strategic value" text="Institutions that implement adaptive systems well may improve outcomes and build a more future-ready learning environment." />
          </div>
        </div>
      </section>
      ...
      <section className="bg-blue-50/70 py-24">
        <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-5">
            {["Canvas", "Blackboard", "DreamBox", "Knewton", "Brightspace"].map((tool) => (
              <div
                key={tool}
                className="rounded-3xl border border-blue-100 bg-white p-6 text-center shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg"
              >
